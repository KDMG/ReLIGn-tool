...
def sgiso_style_output(G: nx.DiGraph, matches):
    out = []
    n_found = len(matches)
    if n_found == 0:
    ...
    for i, m in enumerate(matches, start=1):
        G_nodes = set(G.nodes)
        keys_in_G = G_nodes.intersection(m.keys())
        if keys_in_G:
            nodes_in_match = set(keys_in_G)
        else:
            nodes_in_match = set(G_nodes.intersection(m.values()))

        node_tokens = []
        for n in node_order:
            if n in nodes_in_match:
                lbl = G.nodes[n].get('label', '')
                node_tokens += ['v', str(n), str(lbl)]

        edge_tokens = []
        for u, v in edge_order:
            if u in nodes_in_match and v in nodes_in_match:
                lbl = G.edges[u, v].get('label', '')
                edge_tokens += ['d', str(u), str(v), str(lbl)]
...
def find_instances(graph, pattern):
    ...
    GM = iso.DiGraphMatcher(G1, G2, node_match=node_match, edge_match=edge_match)

    matches = list(GM.subgraph_monomorphisms_iter())
    sub2 = sgiso_style_output(G1, matches)[0]
...
def search_alignment(pattern, dict_trace, graph, dataset):
    if not os.path.isfile(os.path.join(pattern, "alignment.csv")):
        cmd = ['java', "-jar", os.path.abspath(os.path.join('src', 'core', 'big', 'ComputePrecision.jar'))
            , os.path.join(pattern, dataset + '.xes'),
...
def process_g_file(g_file):
    graphs = []
    with open(os.path.join(g_file), 'r') as f:
...
def main(input_data, pattern, dataset, numsub, namesub):
    ...
    graphs = process_g_file(input_data['igs'])
...
if __name__ == '__main__':
    ...
    parser.add_argument("input_data", type=str, help="Name of the dataset to analyse")
    parser.add_argument("pattern", type=str, help="Number of the sub with which the model is to be repaired")
    parser.add_argument("dataset", type=str, help="Number of the sub with which the model is to be repaired")
    parser.add_argument("numsub", type=str, help="Number of the sub with which the model is to be repaired")
    parser.add_argument("namesub", type=str, help="Number of the sub with which the model is to be repaired")
    args = parser.parse_args()
    main(args.input_data, args.pattern, args.dataset, args.numsub, args.namesub) #BPI2017Denied, testBank2000NoRandomNoise
